...
{
  std::cout <<
      "usage: pkgcatalog-scan [--max-document-bytes N]\n"
  ...
  try {
    ...
    for (int index = 1; index < argc; ++index) {
      ...
      if (argument == "--collection") {
        ...
        raw.push_back(raw_collection{std::move(name), std::move(root),
                                     std::nullopt});
        continue;
      }
      if (argument == "--external-revision") {
        ...
        auto [name, revision] =
            assignment(argv[index], "--external-revision");
    ...
    std::cerr << "pkgcatalog-scan: source authority: "
              << value.what() << '\n';
  } catch (const pkgcatalog::error& value) {
    std::cerr << "pkgcatalog-scan: catalog authority: "
              << value.what() << '\n';
