...
for path in README.md HISTORY.md CONTRIBUTING.md MAINTAINING.md Doxyfile \
  DESIGN.md docs/acquisition.md docs/integration.md docs/abi.md \
  TESTING.md docs/code-style.md docs/manpage-markdown.md docs/html.md \
...
for token in 'tests/unit/' 'tests/integration/' 'tests/mechanism/' 'tests/cli/' \
  ...
  grep -F "$token" "$root/TESTING.md" >/dev/null || fail "testing topology drift: $token"
done
